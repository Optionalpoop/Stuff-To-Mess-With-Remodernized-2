...
public class FlamingStorageCubeGUIMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public FlamingStorageCubeGUIMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(StmwrModMenus.FLAMING_STORAGE_CUBE_GUI.get(), id);
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 17 + 8 + sj * 18, 16 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 17 + 8 + si * 18, 16 + 142));
	}
